...
[Features](#-features) • [Quick Start](#-quick-start) • [WiFi Play](#-wifi-multiplayer) • [How to Play](#-how-to-play) • [Project Structure](#-project-structure) • [Tech Stack](#-tech-stack)
...
- ✅ **Smart Bot AI** — priority-based: Win > Capture > Escape > Advance
...
- ✅ **Reconnect** — rejoin a live game using the room code
...
6. Click **Create Room** → share the 6-letter code
...
│   ├── GameRoom.js              # Room lifecycle, timers, bot automation
...
└── public/                      # Frontend (served as static files)
    ...
    │   └── game.css             # Board, HUD, timer, player cards, win overlay
    │
    └── js/
        ...
        ├── board.js             # HTML5 Canvas board renderer (theme-aware)
...
| `game-started` | `{ slotConfig, theme, state }` | Game begins |
| `dice-rolled` | `{ color, value, movable, state }` | Dice result |
| `token-moved` | `{ color, tokenId, captured, win, state }` | Token moved |
| `turn-skipped` | `{ reason, state }` | Turn timed out |
| `timer-start` | `{ duration, color }` | New turn timer |
| `reaction` | `{ name, emoji }` | Player reacted |
| `chat` | `{ name, color, message }` | Chat message |
...
Bot response delay: **800ms – 1.5s** (feels human, not instant).
